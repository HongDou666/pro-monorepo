...
export function base64Encode(str: string): string {
  try {
    ...
    console.error("[Crypto] Base64 编码失败", error);
...
export function base64Decode(str: string): string {
  try {
    ...
    console.error("[Crypto] Base64 解码失败", error);
...
export function xorEncrypt(str: string, key: string): string {
  ...
  try {
    ...
    console.error("[Crypto] XOR 加密失败", error);
 ...
 * @returns 解密后的字符串
 */
export function xorDecrypt(str: string, key: string): string {
  if (!key) return str;
  ...
  try {
    ...
    console.error("[Crypto] XOR 解密失败", error);
...
export async function aesEncrypt(plainText: string, password: string): Promise<string> {
  try {
    ...
    // 导入密钥
    const key = await crypto.subtle.importKey("raw", passwordKey, { name: "AES-GCM" }, false, ["encrypt"]);
    ...
    console.error("[Crypto] AES 加密失败", error);
...
export async function aesDecrypt(cipherText: string, password: string): Promise<string> {
  try {
    ...
    // 导入密钥
    const key = await crypto.subtle.importKey("raw", passwordKey, { name: "AES-GCM" }, false, ["decrypt"]);
    ...
    console.error("[Crypto] AES 解密失败", error);
...
export async function sha256(str: string): Promise<string> {
  try {
    ...
    console.error("[Crypto] SHA-256 哈希失败", error);
...
export async function sha512(str: string): Promise<string> {
  try {
    ...
    console.error("[Crypto] SHA-512 哈希失败", error);
...
export function md5(str: string): string {
  ...
  try {
    ...
    console.error("[Crypto] MD5 哈希失败", error);
